...
import numpy as np
from config import epochs, max_senten_len, LEARNING_RATE, batch_size, dropout
...
def preprocessing_for_bert(data):
    ...
    for sent in data:
        ...
        encoded_sent = tokenizer.encode_plus(
            text=persian_pre_process(sent),  # Preprocess sentence
...
class BertClassifier(nn.Module):
    ...
    def __init__(self, freeze_bert=False):
        ...
        D_in, D_out = 768, len(np.unique(y_train))
...
def train(model, train_dataloader, val_dataloader=None, epochs=epochs, evaluation=False):
    ...
    """
    # Start training loop
    global val_loss_list, train_loss_list
    ...
    for epoch_i in range(epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for b in range(len(batch)):
                batch[b] = batch[b].type(torch.LongTensor)
...
def evaluate_roc(probs, y_true):
    ...
    precision, recall, f_score, _ = precision_recall_fscore_support(y_true, y_pred, average='micro')
    print('micro precision:', precision)
    print('micro recall:', recall)
    print('micro f_score:', f_score)
    precision, recall, f_score, _ = precision_recall_fscore_support(y_true, y_pred, average='macro')
    print('macro precision:', precision)
    print('macro recall:', recall)
    print('macro f_score:', f_score)
    precision, recall, f_score, _ = precision_recall_fscore_support(y_true, y_pred, average='weighted')
    print('weighted precision:', precision)
    print('weighted recall:', recall)
    print('weighted f_score:', f_score)
    plt.show()
    plt.cla()
    plt.title('Loss Value')
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.plot([i + 1 for i in range(epochs)], train_loss_list)
    plt.plot([i + 1 for i in range(epochs)], val_loss_list)
...
if __name__ == '__main__':
    X_train, X_val, y_train, y_val = load_all()
    train_loss_list = []
    val_loss_list = []
    ...
    torch.save(bert_classifier.state_dict(), 'Idin-Bert')
